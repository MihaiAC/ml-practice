...
class BasePruningMethod(ABC):
    ...
    def __call__(self, module, inputs):
        ...
        with torch.no_grad():
            getattr(module, self._tensor_name).data = self.apply_mask(module)
    ...
    def apply_mask(self, module):
        ...
        # apply_mask is called after the model is pruned and before each forward pass
        # after the model is pruned, apply_mask is used to register the "weight" attribute.
        # Changed version:
        with torch.no_grad():
            if getattr(module, self._tensor_name, None) is None:
                orig = getattr(module, self._tensor_name + "_orig")
                pruned_tensor = mask.to(dtype=orig.dtype) * orig
            else:
                curr = getattr(module, self._tensor_name)
                pruned_tensor = mask.to(dtype=curr.dtype) * curr
